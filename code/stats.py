...
def analyze(tweet):
    ...
    bigrams = getBgs(words)
    result['BIGRAMS'] = len(bigrams)
    return result, words, bigrams


def getBgs(tweet_words):
    bigrams = []
...
def geBgsFrequencyDist(bigrams):
    freq_dict = {}
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1], 'r') as csv:
        ...
        for i, line in enumerate(lines):
            ...
            if if_pos:
                ...
                num_neg_tweets += 1
            result, words, bigrams = analyze(tweet)
    ...
    # Bigrams
    bigram_freq_dist = geBgsFrequencyDist(all_bigrams)
