...
def processTweets(csv_file, test_file=True):
    ...
    with open(csv_file, 'r') as csv:
        ...
        for i, line in enumerate(lines):
            ...
            if test_file:
                tweets.append((tweetID, feature_vector))
            else:
                tweets.append((tweetID, int(sentiment), feature_vector))
...
if __name__ == '__main__':
    ...
    tweets = processTweets(TRAIN_PROCESSED_DATASET, test_file=False)
    train_tweets, val_tweets = utils.split_data(tweets)

    print('Training Phase(Extracting features using decisiontree)')
    ...
    print('\n')
    print('Testing the data using decisiontree')
    correct, total = 0, len(val_tweets)
    i = 1
    batch_size = len(val_tweets)
    ...
    clf.modelName = "decision_tree"
    f = open('savedModels/decision_tree.pickle', 'wb')
    pickle.dump(clf, f)
    f.close()
